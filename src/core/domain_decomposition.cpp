...
void dd_init_cell_interactions() {
  ...
  DD_LOCAL_CELLS_LOOP(m, n, o) {
    ...
    ind1 = get_linear_index(m, n, o, {dd.ghost_cell_grid[0], dd.ghost_cell_grid[1], dd.ghost_cell_grid[2]});
    ...
    for (p = lower_index[2]; p <= upper_index[2]; p++)
      for (q = lower_index[1]; q <= upper_index[1]; q++)
        for (r = lower_index[0]; r <= upper_index[0]; r++) {
          ind2 = get_linear_index(r, q, p, {dd.ghost_cell_grid[0], dd.ghost_cell_grid[1], dd.ghost_cell_grid[2]});
          if (ind2 > ind1) {
...
Cell *dd_save_position_to_cell(const double pos[3]) {
  ...
  i = get_linear_index(cpos[0], cpos[1], cpos[2], {dd.ghost_cell_grid[0], dd.ghost_cell_grid[1], dd.ghost_cell_grid[2]});
  return &(cells[i]);
}
...
int dd_append_particles(ParticleList *pl, int fold_dir) {
  ...
  for (p = 0; p < pl->n; p++) {
    ...
    int c = get_linear_index(cpos[0], cpos[1], cpos[2], {dd.ghost_cell_grid[0], dd.ghost_cell_grid[1], dd.ghost_cell_grid[2]});
    CELL_TRACE(fprintf(
        stderr,
