...
std::tuple<Vector<8, std::size_t>, Vector6d> Lattice::map_position_to_lattice(const Vector3d &pos) const {
  int ind[3];
  ...
  return {node_index, delta};
}

/********************** static Functions **********************/

std::tuple<Vector3i, Vector<6, double>>
Lattice::map_position_to_lattice_global(const Vector3d &pos, double tmp_agrid) {
  Vector3d rel;
  auto local_pos = pos;
  ...
  rel = local_pos / tmp_agrid;
  // calculate the index of the position
  std::transform(rel.begin(), rel.end(), ind.begin(),
                 [](int i) { return floor(i); });

  Vector<6, double> delta{};
...
std::tuple<int, Vector3i>
Lattice::map_lattice_to_node(const Vector3i &ind) const {
  ...
  return {map_array_node({grid.data(), 3}),
          {ind[0] - grid[0] * this->grid[0] + this->halo_size,
           ind[1] - grid[1] * this->grid[1] + this->halo_size,
           ind[2] - grid[2] * this->grid[2] + this->halo_size}};
}
